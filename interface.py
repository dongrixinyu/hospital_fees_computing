...
def get_clean_name_from_raw_doctor_name(doctor_name):
    ...
    if '副主任' in doctor_name:
        return doctor_name.split('副主任')[0]

    if '主任' in doctor_name:
        return doctor_name.split('主任')[0]

    if '号' not in doctor_name:  # and '病' not in doctor_name:
        if '专病' in doctor_name:
            return doctor_name.split('专病')[1]
        else:
            return doctor_name
...
def find_all_doctor_name(xls_content):
    doctor_name_index = find_key_item_index(xls_content, keywords='病人医生')

    doctor_name_list = list()
    for line_idx in range(doctor_name_index[0] + 1, xls_content.values.shape[0]):
        res = xls_content.values[line_idx][doctor_name_index[1]]
        if type(res) is str:
            doctor_name_list.append(res)

    return list(set(doctor_name_list))
...
def class_statistics(
        ...
        medical_income_total, sanitation_material_fee, doctor_name_list):
    """ 统计各个类型的计费
    ...
    :param sanitation_material_fee:
    :param doctor_name_list: 有效的医生姓名
    :return:
    """
    ...
    for line_idx in range(doctor_base_index[0] + 1, xls_content.shape[0]):
        ...
        if type(doctor_name) is str:
            if line_idx == doctor_base_index[0] + 1:
                ...
                # 判断该医生名是否应当计入
                if cur_doctor_name in doctor_name_list:
                    is_a_doctor = True
                    cur_doctor_name = get_clean_name_from_raw_doctor_name(cur_doctor_name)
                else:
                    is_a_doctor = False
            ...
            receiving_department_name = xls_content.values[line_idx][receiving_department[1]]
            if receiving_department_name in computing_class:
                tmp_fee = xls_content.values[line_idx][medical_income_total[1]] - xls_content.values[line_idx][sanitation_material_fee[1]]
                print('\t', receiving_department_name, ':\t', tmp_fee)
                tmp_doctor_fee_info.update({receiving_department_name: tmp_fee})
...
class DataStatistics(object):
    def __init__(self):
        ...
        self.doctor_name_list = list()
        self.default_doctor_name_list = list()
    ...
    def get_doctor_name_list(self):
        self.doctor_name_list = list()
        doctor_name_list = sorted(find_all_doctor_name(self.xls_content))

        bianmin_list = sorted([name for name in doctor_name_list if '便民' in name])
        self.doctor_name_list.extend(bianmin_list)

        zhuren_list = sorted([name for name in doctor_name_list if '主任' in name and name not in self.doctor_name_list])
        self.doctor_name_list.extend(zhuren_list)

        no_hao_no_zhuanbing_list = sorted([
            name for name in doctor_name_list if '号' not in name and '专病' not in name and name not in self.doctor_name_list])
        self.doctor_name_list.extend(no_hao_no_zhuanbing_list)

        hao_no_zhuanbing_list = sorted([
            name for name in doctor_name_list if '号' in name and '专病' not in name and name not in self.doctor_name_list])
        self.doctor_name_list.extend(hao_no_zhuanbing_list)

        hao_zhuanbing_list = sorted([
            name for name in doctor_name_list if '号' in name and '专病' in name and name not in self.doctor_name_list])
        self.doctor_name_list.extend(hao_zhuanbing_list)

        no_hao_zhuanbing_list = sorted([
            name for name in doctor_name_list if '号' not in name and '专病' in name and name not in self.doctor_name_list])
        self.doctor_name_list.extend(no_hao_zhuanbing_list)

        self.default_doctor_name_list = list()
        for doctor_name in self.doctor_name_list:
            flag, _ = judge_name(doctor_name)
            if flag:
                self.default_doctor_name_list.append(doctor_name)
...
class SelectFilePage(object):
    ...
    def next_page(self):
        ...
        if self.data_obj.xls_content is None:
            ...
            DoctorNamePage(self.root, self.data_obj)
...
class DoctorNamePage(object):
    ...
    def __init__(self, root, data_obj):
        ...
        title_label = tk.Label(self.interior, text='二. 确定病人医生栏目', font='Helvetica 13 bold')
        title_label.grid(row=horizontal, column=vertical)

        self.doctor_name_check_buttons, horizontal = self._get_sub_item_info(
            horizontal, vertical, bg_color='light blue')
        horizontal += 1
    ...
    def _get_sub_item_info(self, horizontal, vertical, bg_color='light blue'):
        """ 获取每个大类的所有可选子项
        ...
        self.data_obj.get_doctor_name_list()

        check_button_list = list()
        horizontal += 1
        for item in self.data_obj.doctor_name_list:
            check_var = tk.IntVar()
            if item in self.data_obj.default_doctor_name_list:
                cb = tk.Checkbutton(self.interior, text=item, variable=check_var,
                                    bg=bg_color, height=1, width=22)
            else:
                cb = tk.Checkbutton(self.interior, text=item, variable=check_var,
                                    height=1, width=22)
    ...
    def next_page(self):
        ...
        self.data_obj.doctor_name_list = [
            text for button, text in zip(
                self.doctor_name_check_buttons,
                self.data_obj.doctor_name_list) if button.get() == 1]
...
class DetailPage(object):
    def __init__(self, root, data_obj):
        ...
        self.inspection_testing_info_dict = class_statistics(
            self.data_obj.xls_content, self.data_obj.inspection_testing_list, doctor_base_index, receiving_department,
            medical_income_total, sanitation_material_fee, self.data_obj.doctor_name_list)

        self.dermatology_clinic_info_dict = class_statistics(
            self.data_obj.xls_content, self.data_obj.dermatology_clinic_list, doctor_base_index, receiving_department,
            medical_income_total, sanitation_material_fee, self.data_obj.doctor_name_list)

        self.recovery_clinic_info_dict = class_statistics(
            self.data_obj.xls_content, self.data_obj.recovery_clinic_list, doctor_base_index, receiving_department,
            medical_income_total, sanitation_material_fee, self.data_obj.doctor_name_list)
        ...
        self.cosmetic_surgery_info_dict = class_statistics(
            ...
            receiving_department, medical_income_total, sanitation_material_fee, self.data_obj.doctor_name_list)
